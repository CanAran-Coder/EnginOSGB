...
function OurTeamForm({ headers ,data}: { headers: string[],data:ourTeamCardProps[] }) {
    return (<>
        ...
        <div className="shadow-2xl rounded-2xl grid grid-cols-5 border  gap-y-2 py-5 pt-7 shadow-black/20 border-zinc-400 place-items-center items-center overflow-y-auto max-h-160 w-250">
            ...
            {data.map((item,index) => <React.Fragment key={index}>
                ...
                <label><Link target="_blank" href={item.photo_url}>Göster</Link></label>
